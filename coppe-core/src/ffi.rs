mod raw {
    extern "C" {
        // Events
        pub fn subscribe(event_ptr: *const i32, event_len: usize) -> i32;
        pub fn unsubscribe(event_ptr: *const i32, event_len: usize) -> i32;
...
pub fn spawn(command: &str) {
    unsafe {
        raw::spawn(command.as_ptr() as *const u8, command.len());
    }
}
